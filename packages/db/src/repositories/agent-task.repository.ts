...
import {
  ...
  type UpdateAgentTaskPatch,
} from './agent-task-row.js'
import {
  getGlobalAgentTaskLock,
  releaseGlobalAgentTaskLock,
  tryAcquireGlobalAgentTaskLock,
} from './agent-task-lock.js'

export type { AgentTaskRow, CreateAgentTaskInput, UpdateAgentTaskPatch } from './agent-task-row.js'
export { rowToAgentTask } from './agent-task-row.js'
...
export class AgentTaskRepository {
  ...
  getGlobalLock(): { taskId: string; workerId: string; acquiredAt: number } | null {
    return getGlobalAgentTaskLock(this.db)
  }

  /** Acquire global lock; fails if another active task holds it. */
  tryAcquireGlobalLock(taskId: string, workerId: string): boolean {
    return tryAcquireGlobalAgentTaskLock(this.db, taskId, workerId, (id) => this.getById(id))
  }

  releaseGlobalLock(taskId: string): void {
    releaseGlobalAgentTaskLock(this.db, taskId)
  }
}
