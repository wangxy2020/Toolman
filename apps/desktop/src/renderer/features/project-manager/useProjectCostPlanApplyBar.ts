...
export interface ProjectCostPlanApplyBarProps {
  workspaceId: string
  messages: Message[]
  projects: PmProject[]
...
export function useProjectCostPlanApplyBar({
  workspaceId,
  messages,
  projects,
  selectedProjectId,
  ...
  const count = useMemo(() => assignmentCount(costPlan), [costPlan])
