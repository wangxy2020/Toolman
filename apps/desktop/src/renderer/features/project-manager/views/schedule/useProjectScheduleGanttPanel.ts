...
import type { GanttMenuAction, GanttVersionSwitchEntry } from './ProjectGanttMenuBar'
...
import { collectProgressRollupUpdates, buildProgressPercentById } from './pm-gantt-progress-rollup'
import { pmScheduleApi } from './pm-schedule-api'
import {
  buildScheduleUpdateFingerprint,
  escapeCssContentValue,
  parseBaselineNameIndex,
...
export function useProjectScheduleGanttPanel({
  ...
  /**
   * Manual column-count floor for the current project (add/remove columns menu).
   * Reset on project switch so another project's wide layout does not carry over.
   */
  ...
  const handlePrefsChange = useCallback((next: GanttUiPrefs) => {
    ...
    setResourceSlotFloor(next.resourceView.slotCount)
    setCostSlotFloor(next.costView.slotCount)
  }, [])
