...
export function migrateRendererChannelConfigs(
  items: Array<{
    platform: ChannelPlatformId
    enabled: boolean
    name: string
  ...
  for (const item of items) {
    const existing = file.platforms[item.platform]
    if (existing?.appSecretRef || existing?.enabled) continue

    file.platforms[item.platform] = {
      ...
      domain: item.domain,
      allowedChatIds: item.allowedChatIds,
      appSecretRef: item.appSecret.trim() ? encryptSecret(item.appSecret.trim()) : undefined,
      encryptKeyRef: item.encryptKey.trim() ? encryptSecret(item.encryptKey.trim()) : undefined,
    }
    migrated += 1
  }

  if (migrated > 0) writeStoredFile(file)
  return migrated
}
