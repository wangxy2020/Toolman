...
import {
  ...
  rustCommitShippingCiLedger,
  rustExecuteWorkspaceShippingCiWorkflow,
  rustExportErrorAudit,
  rustGetMachineId
} from '@main/services/epcCommercial/rustCli'
import {
  safeWriteProgressCiData,
  SafeWriteProgressCiDataError
} from '@main/services/epcCommercial/safeWriteProgressCiData'
import { safeWriteIpcData, SafeWriteIpcDataError } from '@main/services/epcCommercial/safeWriteIpcData'
import type {
...
import { EPC_WORKFLOW_LOG_HEADER, formatWorkflowLogAppendBlock, workflowLogPathForWork } from '@shared/epcWorkflowLog'
...
export class EpcCommercialService {
  ...
  executeWorkspaceShippingCiWorkflow = async (
    ...
    try {
      ...
      const progressCiJobs = response.report?.progressCiWriteJobs ?? []
      for (const job of progressCiJobs) {
        try {
          ...
          logger.error('safeWriteProgressCiData failed', error as Error)
          return {
            ok: false,
            report: response.report,
            errorCode: 'INTERNAL_ERROR',
      ...
      const jobs = response.report?.alignedIpcWriteJobs ?? []
      for (const job of jobs) {
        try {
          ...
          logger.error('safeWriteIpcData failed', error as Error)
          return {
            ok: false,
            report: response.report,
            errorCode: 'INTERNAL_ERROR',
      ...
      const pendingCommits = response.report?.pendingLedgerCommits ?? []
      if (pendingCommits.length > 0) {
        const commitResult = await rustCommitShippingCiLedger({
          workspaceRoot,
          dataDir: getDataDir(),
          successes: pendingCommits
        })
        if (!commitResult.ok) {
          return {
            ok: false,
  ...
  appendPaymentDataPatch = async (params: EpcPaymentDataPatchParams): Promise<EpcSimpleOkResponse> => {
    ...
    try {
      ...
      logger.error('appendPaymentDataPatch failed', error as Error)
  ...
  applyPaymentDataOverrides = async (workspaceRoot: string): Promise<EpcSimpleOkResponse> => {
    ...
    try {
      ...
      logger.error('applyPaymentDataOverrides failed', error as Error)
      return { ok: false, errorMessage: error instanceof Error ? error.message : String(error) }
    }
  }
